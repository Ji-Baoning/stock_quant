...
| [012 The corporate-action exemption reads the classification](012-corporate-action-exemption-reads-the-classification.md) | accepted | 2026-09-21 | `src/stock_quant/data_pipeline.py`, `src/stock_quant/data_sources/baostock_factor.py`, `src/stock_quant/data_sources/tdx.py` | demotion, exemption conditional, adjustment_observed, VERIFIED_EMPTY settled fallback, baostock factor channel, pytdxdata 0.6.0 | You change which quarantine reasons withhold coverage trust, the coverage fallback semantics, or the price-event channels a verdict may read. |
| [013 The exchange reference price arbitrates a conflict](013-price-observed-arbitration.md) | accepted | 2026-09-22 | `src/stock_quant/data_sources/price_observed.py`, `src/stock_quant/data_pipeline.py`, `src/stock_quant/data_model/corporate_actions.py`, `project/configs/corporate_action_reviews.yml` | price observation, pre_close, reference price, price_observed, tick thresholds, separability, independent verification path, directional skew, fail closed, ADR-007, ADR-014 | You change how a cross-source corporate-action conflict is arbitrated, add a channel to the arbiter chain, or read what a settlement's `confirmed_by` does and does not assert. |
| [014 The float32 floor merges, but only after the channels](014-float32-floor-merge-after-the-channels.md) | accepted | 2026-09-22 | `src/stock_quant/data_model/corporate_actions.py` | float32, one ULP, 丁 class, precision merge, digit count, channel first, ADR-007 ordering | You change how a cross-source corporate-action pair is compared, or when a numeric tolerance may merge one. |
| [015 Raw-snapshot reuse for eligible channels](015-raw-snapshot-reuse-for-eligible-channels.md) | accepted | 2026-09-26 | `src/stock_quant/data_sources/raw_store.py`, `src/stock_quant/data_pipeline.py`, `src/stock_quant/data_model/call_ledger.py` | reuse, REUSABLE_CHANNELS, resolve_reusable, newest candidate, no fallback, allow_empty, raw_snapshot_reuse, baseline_version, call ledger | You change which channels may serve stored raw snapshots back, the reuse refusal rules, or how a round accounts reused versus fetched requests. |
