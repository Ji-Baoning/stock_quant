...
| 未知接口名错误串 | — | 2026-09-12 上午与官方**逐字相同**；**当天晚间起不再相同**（见下方修正）；该用例已由 owner 于同日晚些时候从阶段 0 探针 `CASES` 移出、改派阶段 3 传输保真脚本 |
...
- 取官方**明确报错**的请求，对比错误串。原本指望"未知接口名逐字相同"，
  ...
  自己作答。owner 先显式豁免该差异，后在同日晚些时候把未知接口名这一用例
  **从阶段 0 `CASES` 移出、改派阶段 3 的轴 1 传输保真脚本**（在可控官方额度窗口下
  比对参照错误串；见「修正：未知接口名错误串」），使阶段 0 闸门可稳定跑到退出码 0。
  `classify_error_probe` 的错误串比对机制保留并单测 —— 它是 §3 ④ 的契约 ——
  只是不再被阶段 0 的用例行使。
...
| 未知接口名错误串 | 上午两边**逐字相同**：`请指定正确的接口名`；**晚间起被推翻**，见「修正：未知接口名错误串」一节；该用例已由 owner 于同日晚些时候从阶段 0 探针 `CASES` 移出、改派阶段 3 传输保真脚本 |
...
**处置（owner，2026-09-12，两度改口）**：**第一次**按 §3 ④ 的"owner 显式人工豁免"
记为已豁免差异（该裁定**已被推翻**）；**第二次（现行）**改为把未知接口名这一用例
**从阶段 0 探针 `CASES` 移出、改派到阶段 3 的轴 1 传输保真脚本**
（`verify_transport_fidelity.py`），使阶段 0 闸门可稳定重跑到退出码 0。二次裁定参照的
判定依据：

- 这是网关的**输入校验路径**，不是换源作答 —— 它从不返回数据；
- 阶段 0 探针的核心问题（"官方合法空结果处 relay 是否返回非空"）两个用例全部
  `AGREE_EMPTY`；
- 填充数据此前有 4/4 逐位一致的记录。

**移出阶段 0 不是放弃观察，而是换到能复现的地方观察**（与 `index_daily` 保真检查同一
条理由：官方额度不可控的端点进不了可复现的硬闸门）。阶段 3 的轴 1 脚本须在**可控的
官方额度窗口下**把官方的参照错误串 `请指定正确的接口名` 与 relay 的作答逐字比对，起点
是本节的实测证据。错误串比对机制（`classify_error_probe`）与其单测保留在阶段 0 探针
里 —— 它是 §3 ④ 的契约 —— 只是不再被阶段 0 的 `CASES` 行使。

**这不撤回上面的观测**：网关对不认识的 `api_name` 与官方不一致仍是既有的真实证据，
本节即其记录。`SUBSTITUTION`、以及任何新的或不同的 `ERROR_DIFFERS`，仍是硬阻断。
