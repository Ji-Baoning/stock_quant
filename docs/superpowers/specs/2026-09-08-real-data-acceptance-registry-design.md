...
`operator_id` 是操作者提供的本地标识，仅用于审计，不是密码学签名。`created_at` 使用带时区的 UTC 时间。相同数据由同一操作者在不同时间重复验收，会产生不同记录并全部保留。
...
自动检查只读取指定的不可变数据集及其原始证据，不解析 `CURRENT`，也不联网。
...
`source_status` 只保存来源名、是否必需、成功状态和稳定原因代码；不得保存 Token、请求响应或异常堆栈。`raw_snapshot_sha256` 排序去重，必须能在 `data/raw/` 中找到唯一匹配的原始快照。Bootstrap 数据集标记 `origin=bootstrap`，不能通过 `real-data-v1` 的真实来源验收。
...
命令按 `created_at`、`acceptance_id` 稳定排序，显示全部记录、结论、规则版本和失败原因。不存在记录时明确返回“未验收”，不把缺失解释为通过。
...
2. 重新验证记录内容哈希、数据集清单哈希、质量报告哈希和当前 `policy_version`。
3. 对 `CURRENT_ACCEPTED`，从有效记录中按 `created_at`、`acceptance_id` 稳定选择最近的 ACCEPTED 记录；显式 ID 则只验证该记录，不自动替换。
...
没有有效记录、只有 REJECTED 记录、证据被修改或规则版本过期时，Research 在任何因子/组合/回测计算前失败。失败运行仍保留审计状态和明确原因。

Engineering 模式允许 `data_acceptance_id` 为空并记录 `UNVERIFIED`；即使显式引用有效 ACCEPTED 记录，也维持其 diagnostic-only 的 `UNTRUSTED` 评价规则。
...
- `research/acceptance/service.py`：准备清单、发布判定及 CLI 用例编排。
...
CLI 集成测试覆盖：准备清单、成功发布、失败记录先落盘后非零退出、查询历史、安全输出及明确版本要求。所有测试使用合成数据且默认离线。

Research 集成测试覆盖：

- 无验收记录时在 factor stage 前失败；
