...
首期导入 `csi300` 已取得证据的完整历史，目标为指数创设初期至今。沪深 300 的官方基日为 2004-12-31、发布日期为 2005-04-08；两者都不能自动视为成分事实覆盖起点。正式 `coverage_start` 必须取最早一份通过验收的官方成分证据所明确支持的实际成分生效日，不得写死为 2005-01-01、首个自然年日期或供应商可返回数据的最早日期。覆盖前区间标记为 `OUT_OF_SCOPE`，不是数据缺失；依赖该股票池的策略不得在该区间运行，但因子预热所需的价格历史可以更早。数据集清单记录实际最早/最晚覆盖日，不能把目标当作已达成覆盖。中证 500 的接入只新增定义和事实数据，不复制研究主流程。
...
解析规则：指数纳入早于上市，实际起点取上市日，理由 `before_listing`；调出晚于退市，实际终点取最后可交易日，理由 `after_delisting`；IPO 解禁等限制不改变资格区间，留给交易层；无法证明最后可交易日则正式 Research 拒绝；相交为空时保留事实、标记不可用并使覆盖验收失败。

当前 `security_master` 的正式上市事实来自 `tushare.stock_basic`。其 `delist_date` 的现有来源契约只证明“退市日期”，未明确证明“最后可交易日”，所以不得用于截断资格区间；聚宽 `end_date` 等同样只标为“退市日期”的字段也不满足该门禁。万得、本地文件或未来供应商必须逐来源提交字段语义契约与可核验证据，不能凭字段名称或惯例映射。目前标准主数据表没有独立、已验证的最后可交易日字段；后续若引入，至少保存 `last_tradable_date`、`last_tradable_date_source`、`last_tradable_date_evidence_sha256` 和固定语义 `confirmed_last_trading_day`。只有该独立字段通过验收后，解析层才可用它收窄终点；终止上市公告日、决定日或含糊的退市日期不得猜测。
