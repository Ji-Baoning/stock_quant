...
> **第一条的预期不是"零输出"（2026-09-12 实测）。** 在跑完 7 个任务之后的 HEAD 上，
> `ruff check src project tests` 会报**两条与本计划无关的既有错误**：
> `tests/unit/test_costs.py:12` 的 `I001` 与
> `tests/unit/test_master_bar_boundaries.py:60` 的 `E501`。两者的最后一次改动分别是
> `08e41e4`（2026-09-12）与 `a825f0f`（2026-09-11），都**早于本计划基线 `47bcecf`**，
> 是分支上先于本计划存在的欠账，没有任何一个任务的 diff 碰过这两个文件。
> 因此这一条的判据是「**不新增** lint 错误」：上面两条保持不变、其余为零，即通过。
> 不要为了让这条命令变绿去改那两个无关文件 —— 那会把本计划的改动面扩到不相关的领域，
> 与"每处改动都有任务归属"的约定冲突。要清理它们是另开一轮的事。
