...
1. **停牌回补建模**（推荐方向）：以 `suspend_d` 类停牌事实为证据，为停牌日物化
   `volume=0`、价格按交易所惯例前收平推的 bar 行，并在来源/审计字段如实标注合成属性；
   保持「完整网格」的强保证，下游无需处理洞。
2. **验收口径修订**：引入「经第二来源证实的停牌」可接受分类。保持数据纯净，
   但永久弱化完整性保证，且属 spec 级变更。

无论哪条，都须先成文（新方案）再实施，不得为过检临场放宽。

### 2.3 顺带发现：质量报告存在 56,660 条失真 WARNING（不阻塞）

两次发布的 `quality_report` 均带 56,660 条 `unknown_or_suspended` WARNING，其中包含
实际已发布的 `(symbol, date)`（如 `000001.SZ 2015-01-05`）。定位在 `data_pipeline`
`_missing_issues` 的日期类型比对（基准日与主源日的类型不一致使成员判断恒假），
量级恰好 20 只 pre-2015 上市标的 × 2,833 网格日。该 WARNING 不参与任何门禁判定，
属噪音修复项，随停牌方案一并处理即可。
