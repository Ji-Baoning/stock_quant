...
The dataset-boundary section pins two published versions of one synthetic
market -- v1 with no corporate actions and v2 with one *future* verified cash
dividend -- through the real ``adjusted_bar`` adapter, and proves the action
cannot change any factor row before its ``ex_date``.
"""
