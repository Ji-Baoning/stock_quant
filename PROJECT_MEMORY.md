...
**仍未解锁项**：正式 `research run` 发布（walk-forward / 一次性挑战）仍需 (1) 对携带成员表的数据版本 `b0e36345…` 重走阶段 5b 真实数据验收（操作者人工项，机制就绪）；(2) 全成分行情与公司行为回补——**该子项已于 2026-09-14 执行**：增量规模实为 **659** 只在市成员（`stock_basic` 按 L/D/P 合并 → master 684，`universe.yml` 取其 659 只在市者；「约 654 只」从未由任何脚本或报告推导过，已作废）。原文「`daily`/`adj_factor`/`dividend` 已确认 relay 可得」按现有证据**不成立**：`adj_factor` 只有单样本逐位一致、无 raw 存证，且 `TushareSource` 根本没有这个 endpoint；`dividend` 在 relay 上**零证据**，角色分工设计明列不集成 tushare `dividend`。实际回补路径 = `daily` 走 tushare relay（659/659）+ 公司行为走 akshare 的 cninfo 分红 / eastmoney 分红 / cninfo 配股三条车道（659/659/659，其中 eastmoney 对 5 只科创板标的取数失败 → 该 5 只覆盖窗口 UNTRUSTED，不阻断发布）。因此原先设想的「单源 tushare 分红会整段 `corporate_action_coverage_untrusted`」不再是本路径的问题；`universe_master_mismatch` 契约也**未**放开——走的是把 `universe.yml` 扩到与 master 集合相等的内部一致解。定位到的 5 处数据层缺陷（D1–D5）与 2 处运行级缺陷（D6 配置目录重复 `universe_id`、D7 发布路径要求显式 `TUSHARE_TRANSPORT=relay`）见 `docs/operations/2026-09-14-blocking-gap-root-cause.md`；幸存者偏差语义（25 只已退市成员不在池内）仍待重审。**2026-09-14 定时任务实测确认（详见 `docs/operations/2026-09-14-wf-oos-stage-result-and-diagnostics.md`）**：正式 `walk_forward_oos_v1` 规格在现行 CURRENT `1709eddb…` 上于 `acceptance` 预检 FAILED（`NoValidAcceptance`），且该版本因 ~629 只成员缺行情/公司行为证据在自动检查上两项 FAIL，回补战役落地前不可能验收；钉死已验收的 `52648bdc…` 则在 `universe_acceptance` 失败（空成员表无法匹配冻结定义）。短路径 = 对 `b0e36345…` 完成最后一项人工确认（`cross_source_price_sample`，清单已 8/9 确认、standing worksheet 就绪）并 publish；绑定该版本的冻结定义已恢复为 `configs/universes/custom_csi300_tw_tradable_28.yml`（universe_version 逐位复现 `c211b85c…`），配套规格模板见运维文档。同日新增解释性诊断脚本 `project/explanatory_diagnostics.py`（逐年/板块 IC、五分位多空、暴露、集中度、参与率、拒单压力、成本归因），对 `46f8b74e…` 的结论：亏损年份（2015/2018/2022/2026YTD）= 多头 beta 定基调 + 恰好信号失效（IC 转负、Q5-Q1 转负）放大，集中度/流动性/执行均非成因，成本为稳定次要拖累（滑点>印花>佣金，每笔 10.2bp 为申报假设非实测）。**死资产退场（2026-09-14）**：`index_constitution`（ic）谱系的两个宇宙定义 `custom_csi300_ic` / `custom_csi300_ic_tradable` 已退场，绑定它们的三个规格 `momentum_60d_pit_tradable.yml`、`momentum_60d_wf_real_baseline.yml`、`momentum_60d_wf_real_challenger.yml` 随之退场。**落盘状态核对（2026-09-15）**：三个规格已**真正**移入 `project/configs/experiments/archive/`——在此之前，两个归档 README 与本段都写着「已停泊」，而磁盘上三个规格仍在 `configs/experiments/` 顶层，即文档描述的是一次从未发生的搬移，现以文件系统为准；两个 ic 定义**仍未**移入 `project/configs/universes/archive/`，依旧留在 `configs/universes/` 顶层，与 `universes/archive/README.md` 的「sit here／不可换回」断言不符。未顺手搬走定义的原因：定义会进入 `load_universe_coverage_criterion` 的扫描集合并被逐条记入数据版本 manifest 的 `universe_coverage_definition_hashes`（现行 `1d6e43b4…` 里正记着这两个 id；实测四者版本与该记录逐条相等，扫描 `acceptance_start` 恒为 2015-01-05），故搬走不会让任何校验失败，却会改变下一次发布的 manifest 内容，属构建输入变更，需操作者明确决定，不并入本次零风险清理。判据：两个定义钉住的 `membership_table_sha256`（`a7ee5431…` / `756db257…`）在 `data/standardized/` 的 14 个已发布数据集上**无一匹配**，绑定它们的运行一律在 universe 预检阶段、任何因子或回测计算之前失败；09-11 记录的那次实验所用数据集 `38c52358…` 已不在磁盘。原始证据（`data/raw/csi/custom_csi300_ic_membership_snapshot.csv`、`data/raw/csi/index_constitution/`、`data/membership/custom_csi300_ic.parquet`）保留，但复活该谱系需完整重新导入并重发数据集，不是文件换位。活跃谱系为 `custom_csi300_tw` / `custom_csi300_tw_tradable`（后者绑定现行 CURRENT `1d6e43b4…`，`universe_version 1d6a8c8f…`；该定义钉住的成员表哈希 `5f5bf476…` 于 2026-09-15 实测等于 `1d6e43b4…` 的 766 行 `universe_membership` 内容哈希）；`project/verify_update_readiness.py` 与 `project/trim_universe_membership.py` 的默认常量已随之更正。**记载更正**：`docs/operations/2026-09-14-wf-oos-stage-result-and-diagnostics.md` §1.3 称 `momentum_60d_pit_official.yml` 引用死定义 `custom_csi300_ic_tradable`，实际该规格绑定的是活跃的 `custom_csi300_tw_tradable`，不属死资产之列。**挑战者缺口已补（2026-09-15）**：新增 `project/configs/experiments/momentum_60d_wf_tw_challenger.yml`——与 `momentum_60d_wf_tw_baseline.yml` 加载后逐字段比对，**唯一差异为 `portfolio_rule` 与 `hypothesis`**（`top_n_equal_weight` vs `buffered_risk_weighted`，后者用全部冻结默认参数），满足挑战服务对配对规格的要求；`universe_definition` 绑定 `custom_csi300_tw_tradable`，`execution_pipeline` / 日期范围 / 成本情景顺序 / 因子版本 / 初始资金与基线完全一致，故 universe 预检可过。**写规格不消费任何东西**：holdout 注册表仍未被消耗，挑战者快照哈希仍在其正式运行时才冻结；挑战侧两半至此齐备，仍待操作者按 RUNBOOK 阶段 8 在真实数据上执行一次。

下列原始解锁路径记录（2026-09-10）保留备查：tushare `index_weight`（官方指数成分权重月度快照）是当前最可行的时点成员证据源，但需要 2000 积分档 token（现有 token 无该接口权限，`index_daily` 的 1 次/小时限速同为低积分表现）。`project/collect_index_weight_membership.py` 已备好端到端流水线：逐月拉取快照存证 `data/raw/csi/index_weight/` + 证据清单哈希 → 合并快照 CSV（_attested-boundary 窗口约定：成员保持至首个不再列出它的快照前一日，新成员自首个列出它的快照日起，月末快照对月中调仓有至多约 1 个月的滞后，已文档化）→ 共享证据绑定导入 → 数据集重发布带成员表 → 生成冻结宇宙定义 YAML。默认 `custom_csi300_tw`（custom 前缀豁免每日恰 300 只的基数校验，证据链要求完全相同；若另持官方调仓生效日证据可改用 `csi300`）。换上有权限 token 后一条命令跑通，再按阶段 5b 完成真实数据验收，然后才能预注册执行阶段 8 的一次性挑战。工程信任模式另有设计约束：`research run` 恒为 RESEARCH 模式（无绕过），工程诊断只走 `backtest --engineering`；`buffered_risk_weighted` 只在 walk-forward 管线执行、单窗口工程管线会响亮拒绝。
...
- 架构事实层为 `docs/architecture/`（定位、模块图、数据流、不变量），随代码同步
  修正；已采纳决策及其取舍记录在 `docs/adr/DECISIONS_INDEX.md`，ADR 一经采纳不
  原地改写结论，只能由新 ADR 取代。
- 本文件、README 与 RUNBOOK 可链接到上述两层，但不再作为架构契约的唯一来源；
  `docs/superpowers/` 下的历史规格与计划仅作追溯，不是当前事实的默认来源。
